...
class ConfigObject(object):
    def __init__(self, client, obj_read_func = None, obj_list_func = None):
        self.client = client
        self.vnc = client.vnc
    ...
    def fqn_to_str(self, fq_name):
        fqn_str = ''
        for item in fq_name:
            fqn_str += item + ':'
        return fqn_str.rstrip(':')
    ...
    def list(self, args):
        ...
        for item in dict[dict.keys()[0]]:
            print self.fqn_to_str(item['fq_name'])
            continue
...
class ConfigCni(ConfigObject):
    def __init__(self, client):
        super(ConfigCni, self).__init__(client) 

    def add_cni(self, tenant, id, vm, network, veth):
        # VMI
        agent_vmi = ConfigPort(self.client)
        vmi_args = ConfigArgs()
        vmi_args.name = 'id'
        ...
        vn_id = self.vnc.fq_name_to_id('virtual-network',
                ['default-domain', self.tenant.name, network])
...
class ConfigInstanceIp(ConfigObject):
    ...
    def create(self, args):
        try:
            vn_fqn = ['default-domain', self.tenant.name, args.network]
...
class ConfigVm(ConfigObject):
    def __init__(self, client):
        super(ConfigVm, self).__init__(client,
...
class ConfigVirtualRouter(ConfigObject):
    def __init__(self, client):
        super(ConfigVirtualRouter, self).__init__(client,
...
class ConfigLoadbalancer(ConfigObject):
    def __init__(self, client):
        super(ConfigLoadbalancer, self).__init__(client,
...
class ConfigLoadbalancerListener(ConfigObject):
    def __init__(self, client):
        super(ConfigLoadbalancerListener, self).__init__(client,
...
class ConfigLoadbalancerPool(ConfigObject):
    def __init__(self, client):
        super(ConfigLoadbalancerPool, self).__init__(client,
...
class ConfigLoadbalancerMember(ConfigObject):
    def __init__(self, client):
        super(ConfigLoadbalancerMember, self).__init__(client,
...
class ConfigServiceApplianceSet(ConfigObject):
    def __init__(self, client):
        super(ConfigServiceApplianceSet, self).__init__(client,
    ...
    def delete(self, args):
        fqn = ['default-global-system-config', args.name]
        try:
            self.vnc.service_appliance_set_delete(fq_name = fqn)
        except Exception as e:
            print 'ERROR: %s' %(str(e))

class ConfigServiceAppliance(ConfigObject):
    def __init__(self, client):
        super(ConfigServiceAppliance, self).__init__(client,
...
class ConfigPort(ConfigObject):
    ...
    def delete_old(self, name, network, address, sg):
        update = False
        obj = self.obj_get(name, msg = True)
        if not obj:
            return
